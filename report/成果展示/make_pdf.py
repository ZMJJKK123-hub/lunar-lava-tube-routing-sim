...
# ---------- 第 2 页: 信道三臂 (先讲清比什么, 再上图) ----------
pdf.add_page()
head("强化学习实验① · 消息走哪条\"频道\"由谁来选",
     "A=手写规则 B=机器学习(Q-learning) C=掷骰子(对照组) | 同地图同消息流, 5 轮配对, 统计检验")
paras(32, [
    "先说比的是什么: 一条消息要从桩子 A 送往桩子 B, 中间每一跳往往有多条链路可选, "
    "选得差就绕路、撞干扰, 消息变慢。我们只比一个最直白的量——消息从发出到送达的平均耗时 "
    "(仿真 1 拍=0.25 秒, 图中已换算成秒)。",
    "三个\"脑子\"同台: A=人工手写规则, 凭经验把选频优先级一次定死; "
    "B=机器学习 (Q-learning), 每根桩只看自己身边的链路质量来选频道, 消息送完按快慢给自己打分, "
    "边干边学; C=掷骰子纯随机——对照组, 专门用来证明 B 若赢了靠的是\"学到了\", 不是手气好。",
    "公平比法: 同一张地图、同一批消息流, 三组各跑 5 轮、出场次序轮换, 差距做过统计检验才认账。",
], size=9.8, lh=5.6)

y = pdf.get_y() + 10
pdf.image(c3, x=14, y=y, w=182)
y = y + 182 * 3.6 / 6.8 + 8
howto(14, y, 182,
      "怎么看上图: 左图比三种脑子的平均送达耗时 (误差杠=5 轮的波动); "
      "右图逐轮比学习组与随机组——5 根柱子全在零线上方=学习组轮轮更快, 不是运气。")
section(y + 15, "▎两个核心发现")
pdf.set_xy(14, y + 26)
pdf.set_font("yh", "", 9.8)
pdf.set_text_color(60, 75, 95)
pdf.multi_cell(182, 6,
               f"① 学习组最快: 平均 1.73 秒送达, 比手写规则快 0.10 秒、比随机快 0.10 秒。统计检验值 t={abs(t_bc['t']):.1f}"
               " (只要超过 2.78 就算\"差距是真的\"), 5/5 轮全胜——赢在实力不是运气。\n"
               "② 对照组揭了规则的底: 手写规则组和掷骰子组速度没有显著差别 (t=0.3) —— 费心手写的选频规则"
               "其实没比瞎选强, 真正有效的只有\"边干边学\"。", align="L")
banner(276, "一句话: 只凭本地观察和真实奖惩, 分布式学习稳定跑赢了人工调参的全局规则。")
