...
# ---------------- 路由粘滞 (routing.py 消费) ----------------
ROUTE_KEEP_BAND = 0.12      # 旧路径保留带宽: 代价仍在最优的 +12% 内不翻转
ROUTE_HOLD_TICKS = 20       # 路径保持期: 刚换过的路径 20 拍内除非断裂不再换
                            # (双保险: 自身流量拥塞会让自己显得贵, 纯带宽判不住)
...
# ---------------- 移动干扰源 (engine/api 开关式灾害; physics 消费) ----------------
JAM_RADIUS = 500.0          # 干扰有效半径 (世界米): 接收端距源凸衰减至 0
JAM_LIFT_MAX_DB = 30.0      # 中心最大噪声抬升 (dB): 圈内 SNR<38dB 的链路近中心全灭
JAM_SPEED = 45.0            # 干扰源游走速度 (世界单位/tick)
