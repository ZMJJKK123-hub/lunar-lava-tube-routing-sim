...
(决策在 robot.py 的状态机; SOS 布防/解除判定在 sos.py)。
...
from .constants import (RANGE, ROBOT_CHAIN_INTEL, STALE_AFTER,   # 情报开关/超时
...
class SenseMixin:
    ...
    self._checked_until (核查冷却表), self.trail (面包屑),
    self.state/self.target (任务状态)。

    调用链: _advance -> _hear (实时) / _chain_intel (滞后情报)
    -> _start_mission (开任务); 呼救布防/解除见 SosMixin (sos.py)。
    """
    ...
    def _hear(self):
        ...
        尊重目标冷却表 (_giveup 登记): 刚放弃过的目标不再派单 —— 否则
        单钉救不了的目标会每个耳聋期一趟, 无限白跑 (与链上情报同口径)。"""
        best, bd = None, RANGE
        for nid in self.sos_active:
            if self.eng.tick < self._checked_until.get(nid, 0):
                continue               # 目标冷却中: 刚因不可救/超时放弃过
